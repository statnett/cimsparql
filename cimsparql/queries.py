...
def node_delta_power(node: str, cim_version: int) -> str:
    variables = [f"?{node} (sum(?inj) as ?delta_p)"]
    where_list = [
        common_subject(
            "?_t_mrid",
            [
        ...
        common_subject(
            "?_obj",
            ["cim:SvPowerFlow.Terminal ?_t_mrid", "cim:SvPowerFlow.p ?p"],
        ),
        f"?_{node} {ID_OBJ}.mRID ?{node}",
        "bind(xsd:float(?p) as ?inj)",
    ]
    return sup.groupby(variables, where_list, f"?{node}")
...
def load_query(
    ...
    where_list = [
        common_subject(mrid_subject, [f"{ID_OBJ}.mRID ?mrid", f"{ID_OBJ}.aliasName ?name"]),
...
def synchronous_machines_query(
    ...
    where_list = [
        common_subject(
            mrid_subject,
            [f"{ID_OBJ}.mRID ?mrid", sup.get_name("", name), sup.rdf_type_tripler("", SYNC_MACH)],
        ),
        *sup.bid_market_code_query(mrid_subject),
        f"?_t_mrid {ID_OBJ}.mRID ?t_mrid",
        sup.terminal_where_query(
