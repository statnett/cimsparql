...
def winding_from_three_tx(three_tx: pd.DataFrame, i: int) -> pd.DataFrame:
    columns = [col for col in three_tx.columns if col.endswith(f"_{i}") or col == "p_mrid"]
    winding = three_tx[columns]
    t_mrid = f"t_mrid_{i}"
    rename_columns = {
        column: "_".join(column.split("_")[:-1])
        for column in columns
        if column not in [t_mrid, "p_mrid"]
    }
...
def three_tx_to_windings(three_tx: pd.DataFrame, cols: List[str]) -> pd.DataFrame:
    ...
    windings["mrid"] = windings["w_mrid"]
    windings["t_mrid_2"] = windings["p_mrid"]
...
def windings_set_end(windings: pd.DataFrame, i: int, cols: List[str]):
    ...
    return windings[windings["endNumber"] == i][["p_mrid"] + cols].rename(columns=columns)
...
def windings_to_tx(
    ...
    three_winding_mrid = windings[windings["endNumber"] == 3]["p_mrid"]
    three_windings = windings.loc[windings["p_mrid"].isin(three_winding_mrid)]
    wd = [windings_set_end(three_windings, i, cols).set_index("p_mrid") for i in range(1, 4)]
    three_tx = pd.concat(wd, axis=1, sort=False)

    # While two winding transformers don't. Combine first and second winding.
    two_tx_group = windings[~windings["p_mrid"].isin(three_winding_mrid)].groupby("endNumber")
    two_tx = two_tx_group.get_group(1).set_index("p_mrid")
    two_tx_2 = two_tx_group.get_group(2).set_index("w_mrid")
    for col in ["t_mrid", "bidzone"]:
        two_tx.loc[two_tx_2["p_mrid"], f"{col}_2"] = two_tx_2[col].values
    two_tx.loc[two_tx_2["p_mrid"], "x"] += two_tx_2["x"].values

    if not phase_tap_changers.empty:
        phase_tap_changers["w_mrid_1"] = two_tx.loc[
            two_tx_2.loc[phase_tap_changers.index, "p_mrid"], "w_mrid"
        ].values

    two_tx = two_tx.reset_index().rename(
        columns={"w_mrid": "mrid", "t_mrid": "t_mrid_1", "bidzone": "bidzone_1"}
    )
